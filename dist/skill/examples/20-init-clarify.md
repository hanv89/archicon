...
only the single un-resolved question (Q5, format).

## Case A — ambiguous prompt → Q3 + Q4 + Q6 fire

> **User:** I need a diagram for the payments service.

Prompt analysis: no vendor named (→ Q3 must fire); no diagram-type word or
audience signal (→ Q4 must fire — Q1+Q2 don't resolve a type via the 4×4
table because audience is unknown); no create-vs-edit verb (→ Q6 must fire).
Q5 has a sensible default (one-click render URL for new diagrams), agent
...
## Case B — clear create-verb prompt → only Q5 fires
...
(vendor=Azure); "Container diagram" → skip Q4 (level=Container); no format
hint mentioned → Q5 must fire OR agent applies the default (one-click render
URL). Agent picks the latter — single-question batteries are friction; the
default is sensible enough to skip the ask.
